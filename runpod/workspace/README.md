...
## 7. ComfyUI が開けない / 403 が出るとき

ページに「**アクセスが拒否されました / このページを表示する権限がありません（HTTP ERROR 403）**」
と出る場合、**Pod は正常で、ログインしているアカウントが原因**です。

### まず10秒で切り分ける

**シークレットウィンドウ（プライベートウィンドウ）で同じURLを開く。**

| 結果 | 意味 |
|------|------|
| **開ける** | アカウントの問題。下記の対処へ（急ぐならシークレットのまま使ってOK） |
| 開けない | Pod が停止しているか、URL が古い可能性。「Start できないときは作り直す」を参照 |

### 対処

1. `console.runpod.io` → 右上のアイコン → **Logout**
2. **チームに招待されたメールアドレス**でログインし直す
   （Google アカウントを複数使い分けている場合、ここを間違えがちです）
3. 右上のアイコンから **チーム「Chammi」に切り替える**

個人アカウントのままだと **Pod が1台も表示されません**。それが見分け方です。

### なぜ「シークレットなら見られる」のか

RunPod のプロキシ URL は、**ログインしていなければ誰でも開けます。**
セッションがある場合だけアカウントの権限を見て、権限が無ければ拒否します。

つまり「権限が足りない」のではなく、**別のアカウントでログインしていたために弾かれていた**、
という状態です。

> ⚠ 裏を返すと、**この URL を知っている人は誰でも ComfyUI を開けます。**
> 社外に共有しないでください。
